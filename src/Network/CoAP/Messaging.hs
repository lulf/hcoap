...
handleResponse :: Message -> Res.ResponseCode -> MessageStore -> IO ()
handleResponse _ _ _ = return ()

handleEmpty :: Message -> MessageStore -> IO ()
handleEmpty message store = return ()
...
recvRequest sock store = do
  ...
  return (case code of
         ...
         Response responseCode -> do
           handleResponse message responseCode store
